export default ({type, lable, placeholder, fieldRef, hassError}) => {
    ...
    return(
        <div className="form-group mb-6">
            <label className="block text-gray-700 text-sm font-bold mb-2">{lable}</label>
            { type === 'textarea' ? (
            ...
            {hassError && <p className="text-red-500 text-xs italic">{`${lable} is required`}</p>}
        </div>
    );
};